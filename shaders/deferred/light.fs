...
vec3 CalcDirLight(DirLight light, vec3 color, float spec, vec3 normal, vec3 viewDir)
{
    ...
    float specCalc = pow(max(dot(viewDir, reflectDir), 0.0), 128.0f);
    // combine results
    vec3 ambient  = light.ambient  * color;
    vec3 diffuse  = light.diffuse  * diff * color;
    vec3 specular = light.specular * specCalc * color * spec;
...
vec3 CalcPointLight(PointLight light, vec3 color, float spec, vec3 normal, vec3 fragPos, vec3 viewDir)
{
    ...
    // specular shading
    vec3 reflectDir = reflect(-lightDir, normal);
    float specCalc = pow(max(dot(viewDir, reflectDir), 0.0), 32.0f);
    // attenuation
